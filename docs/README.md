...
| [PHASES.md](./PHASES.md) | **Phased build plan** — **Phase 1** heartbeat only (committed baseline); **from Phase 2**, app **and** Terraform grow together; **Phase 5** uses **Docker → ECR → Lambda (container)** + Terraform **`plan`** (then optional **`apply`**), then **Open-Meteo** + more IaC → hardening. |
