...
- **`variables.tf`**, **`outputs.tf`**, **`versions.tf`** — configuration surface and provider pins. Phase 7 surfaces the outputs the manual deploy runbook references: **`ecr_repository_url`**, **`lambda_function_name`**, **`lambda_alias_name`**, plus a placeholder **`api_base_url`** that is `null` until the API fronting slice (API Gateway HTTP API or Lambda Function URL) lands.
